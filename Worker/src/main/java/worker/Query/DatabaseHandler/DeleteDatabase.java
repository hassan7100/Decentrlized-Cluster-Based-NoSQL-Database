...
@Slf4j
@Component
public class DeleteDatabase implements DatabaseHandler {
    @Autowired
    ...
    public Object handleQuery(Query query) {
        ...
        Status status = databaseService.dropDatabase(query.getDatabaseName());
        log.info("User: "+query.getUsername()+" tried to delete a database named:"+query.getDatabaseName()+", status: "+status.getStatusType());
